  ...
  <div class="py-2 w-full">
    <div class="flex items-center justify-between">
      <div class="flex items-center text-white">
        <img class="rounded-full h-[35px]" :src="post.image" alt="Post Image" />
      ...
      <div class="relative" @click="isMenu = !isMenu">
        <button
            ...
            class="flex items-center text-white p-1 size-[24px] hover:bg-gray-800 rounded-full cursor-pointer">
          <Icon v-if="!isDeleting" name="bi:three-dots" color="white" size="18" />
          <Icon v-else name="eos-icons:bubble-loading" color="white" size="18" />
        </button>
    ...
    <div class="relative flex items-center w-full">
      <div class="w-[42px] mx-auto">
        ...
        <div class="bg-black rounded-lg w-[calc(100%-50px)] text-sm font-light mb-2">
          <div class="py-2 text-gray-300">{{post.text}}</div>
          <img v-if="post && post.picture" :src="post.picture" class="rounded mx-auto w-full mt-2 pr-2" alt="Post Image" />
          <div class="absolute mt-2 w-full ml-2">
            <button
            :disabled="isLike"
            class="flex items-center gap-1"
            >
              <Icon name="mdi:cards-heart-outline" size="28" class="p-1 text-white hover:bg-gray-800 rounded-full cursor-pointer" />
            </button>
